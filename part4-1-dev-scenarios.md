# VMware Private AI Foundation 가이드 - Part 4-1
## 개발 환경별 시나리오 (7장)
...
> **⚠️ 주의:** 시나리오 A는 개발/테스트 환경에서는 유연하지만, 프로덕션 전환 시 컨테이너화, 오케스트레이션, 모니터링 등 상당한 추가 작업이 필요하므로 DLVM only 는 권장하지 않습니다.
...
**시나리오 C**는 PAIS의 Model Endpoint만 활용하고, RAG 파이프라인은 직접 구현하는 방식입니다. 모델 서빙의 편의성과 RAG 커스터마이징의 유연성을 동시에 얻을 수 있습니다. 하지만 아키텍처가 복잡해지고 관리 난이도가 높아집니다.
